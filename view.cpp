...
gQGraphicsView::gQGraphicsView(QGraphicsScene *scene)
    : QGraphicsView(scene)
{
...
//            scene()->addRect(point.x(), point.y(), 36, 18);
            QPainterPath pp;  pp.moveTo(point);
            pp.addEllipse(point, 12, 12);
            pp.addText(point.x(), point.y(), QFont("Arial", 12), " point 1");
            QGraphicsPathItem *path = scene()->addPath(pp, QPen(Qt::yellow), QBrush(Qt::yellow));
